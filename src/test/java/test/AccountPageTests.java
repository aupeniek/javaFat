...
public class AccountPageTests extends TestWrapper {
	...
	@Test
	public void addKeyFromSummaryPage(){
		Common.clickNavigationLink("My Account");
		accountPage = page(AccountPage.class);
		System.out.println("Adding key from account summary page");
		SelenideElement keylist = $(".span12 div.row-fluid div.row-fluid");
		
		if (accountPage.keyExists(keyName, keylist)) {
			accountPage.openSshKeysTab();
