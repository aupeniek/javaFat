...
public class AccountPage {
	...
	/**
	 * Method for validating key and key name info under My Account Summary SSH Public
	 * Keys section.
	 * 
	 * @param keyName
	 * @param sshKey
	...
	public void validateSshKey(String keyName, String sshKey) {
		...
		el.$("span.pointer span.title").click();
		el.$(".row-fluid", 1).$(".value").should(hasText(sshKey));
		el.click();
	}

	/**
	 * Method for validating key and key name info under My Account SSH Keys
	 * sub-page.
	 * 
	 * @param keyName
	 * @param sshKey
	 ...
	 * @return SelenideElement of the row in given container
	 */
	public boolean keyExists(String keyName, SelenideElement holder) {
		ElementsCollection c = holder.$(".item-list-container").$$(
				"div.item.row-fluid");
		for (SelenideElement el : c) {
			if (el.text().matches("(.*)" + keyName + "(.*)")) {
